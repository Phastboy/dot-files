...
parse_git_branch() {
  ...
  if [ -n "$branch" ]; then
    ...
    local ahead=$(git rev-list "HEAD..$upstream" --count 2> /dev/null)
    local behind=$(git rev-list "$upstream..HEAD" --count 2> /dev/null)
    local result=" ($branch"
...
if [ "$color_prompt" = yes ]; then
  ...
  PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]:\[\033[01;34m\]\w\[\033[00m\]$(parse_git_branch)\$ '
fi
